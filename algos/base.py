...
class BaseAlgo(ABC):
    ...
    def calculate_lamda_with_cost(self, N):
        ...
        for i in range(N):
            ...
            while not done:
                action = [acmodel(obs[i]) for i, acmodel in enumerate(self.acmodels)]
                obs, reward, done, info = self.env.step(action)
                count += 1
                for j in range(self.agent_num):
                    key = obs[j]['image'].tobytes()

                    if lambdas[j].get(key):
                        lambdas[j][key] += 1
                    else:
                        lambdas[j][key] = 1
